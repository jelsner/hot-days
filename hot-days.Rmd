...
We examine the statistics of extremely hot days in Tallahassee, Florida. An extremely hot day is defined as one in which the high temperature exceeds 100$+^\circ$ F. An extremely hot night is defined as one in which the low temperature fails to drop below 77$+^\circ$ F. A record of daily high and low temperatures from the regional airport reveals an increasing trend in the occurrence of such days since the 1970s. The trend results from an increase in the occurrence of hot events and an increase in the average length of an event. We speculate that the trend is related to changes in the intensity and location of deep layer ridging, perhaps exacerbated by changes in land use and global warming.
...
```
###Figure: The number of days by temperature. By high temperature (top) and by low temperature (bottom).

The most commn high temperature is 90F and the most common low temperature is 72F. The modes are close to the right-tail extremes (high: 100+, low: 77+).
...
```
###Figure: Number of times by day of the year the daily high reached 100F (top) and number of times the daily high remained at or above 77F.
...
Yearly.df <- TLH.df %>% 
  group_by(Year) %>%
  summarize(MaxCount  = sum(TMAX >= 100, na.rm = TRUE),
            MinCount = sum(TMIN >= 77, na.rm = TRUE))
...
What does an annual rate of .49 mean? It means that we can expect a 49% chance of at least one 100F day per year. Or a 1 - (1 -.49) * (1-.49) * (1-.49) = 86.7 chance of at least one 100F day every 3 years. 

What does the coefficient on the Year term mean? Ten years from now there will be a 1 - (1 - .49 * 1.02^10) ^ 3 = 93.5% chance of at least one 100F day every 3 years.
...
Consecutive hot days tend to result in the hottest days. As a consequence of this clustering, it is useful to consider hot events. Here a hot day event is defined as one or more days in which the temperature reaches at least 100F. An event might consist of a single day or it may consist of several consecutive days (heat wave).

Determine hot day events. REPEAT FOR hot nights.
```{r}
...
# empirical chance of another 100+F day
sum(LI.df$eventLength > 1)/length(LI.df$eventLength) 

cor(LI.df$eventLength, LI.df$maxEventT, method = "s")
...
AllYears <- data.frame(Year = 1950:2018)
...
ggplot(data = LI3.df, aes(x = Year, y = avgEL)) +
  geom_point() +
  stat_smooth(method = "glm",
