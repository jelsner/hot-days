...
Last hot days? Hottest days. Hot days since 2009
```{r}
TLH.df %>% 
...
( p1 <- ggplot(out, aes(x = TMAX, y = prob100, group = I)) +
...
What is the likely number of 100 degree days at the airport over the period of record when the daily high temperature was recorded only downtown?
```{r}
n100 <- numeric()
for(i in 1:1000){
...
Time series of counts correct vs uncorrected.
```{r}
YC.df <- YC.df %>%
...
( p1 <- ggplot(YC.df, aes(x = Year, y = nHotDays, fill = nHotDays)) + 
  geom_bar(stat = 'identity', color = "white") + 
  stat_smooth(method = "glm.nb",
              formula = y ~ x, 
              se = FALSE) +
  scale_fill_continuous(low = 'orange', high = 'red') +
  scale_color_manual(values = c("orange", "gray70")) +
#  geom_text(aes(label = nHotDays), vjust = -.5, size = 3) +
  scale_y_continuous(limits = c(-5, 20), position = "left") +
  scale_x_continuous(breaks = seq(1890, 2020, 10)) + 
  geom_vline(xintercept = 1940, col = "gray70") +
  ylab("Number of Days") +
