...
( main.tm <- 
    ...
    tm_shape(TLH_site.sf) +
       tm_symbols(shape = 19, size = .1, col = "orange") +
 #      tm_text(text = "name") + 
...
quantile(TLH.df$TMAX, probs = c(.95, .99, .996))
quantile(TLH.df$TMIN, probs = c(.95, .99, .996))
```

The 99.6th percentile of the distribution of daily high and low temperatures in the observations. 4 days in 1000.
...
ggplot(FreqByT, aes(x = TMAX, y = perc, fill = Epoch)) +
  geom_bar(stat = 'identity', col = 'white', position = "dodge") +
#  geom_bar(data = FreqByT2, stat = 'identity', fill = "orange", position = "dodge") +
  scale_x_continuous(breaks = seq(40, 100, 10),
                     labels = paste0(seq(40, 100, 10), "°")) +
...
#  geom_line() +
  stat_smooth(method = "loess", formula = y ~ x, span = .33, size = 1, se = FALSE) +
...
Compare with the number of 90F days. The upward trend starts at 95F. No changes in temps lower than that. This contrasts with the standard model.
```{r}
TLH.df %>% 
#  filter(Month >= 5 & Month <= 9) %>%
  group_by(Year) %>% 
...
Raster map. Close up.
```{r}
Sites.df <- data.frame(Name = c("TLH", "DTN"), 
                       Latitude = c(30.39306, 30.43333),
                       Longitude = c(-84.35333, -84.28333))
```
...
p <- ggmap(get_googlemap(center = c(lon = -84.315, lat = 30.41),
                    zoom = 13, scale = 2,
                    maptype ='satellite',
                    color = 'color'))
p + geom_point(aes(x = Longitude, y = Latitude), 
               data = Sites.df, size = 2, col = "orange") +
...
```
###Figure: Map of Tallahassee showing the locations of the two temperature records.
