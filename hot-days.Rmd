...
ggarrange(p1, p2, 
          nrow = 2, 
          labels = c("Number of Days by High Temperature", 
                     "Number of Days by Low Temperature"),
...
( p1 <- ggplot(YC.df, aes(x = Year, y = nHotDays, fill = nHotDays, color = LabelF)) + 
  stat_smooth(method = "glm.nb",
              formula = y ~ x, 
              se = FALSE) +
...
p1 <- ggplot(YC.df, aes(x = Year, y = nHotNights, fill = nHotNights, color = LabelF)) + 
  geom_vline(xintercept = 1940, col = "gray70") +
  stat_smooth(method = "glm.nb",
              formula = y ~ x, 
              se = FALSE) +
...
ggplot(FreqByT, aes(x = Temp, color = Epoch)) +
  geom_density(bw = 1, size = 1) +
  geom_hline(yintercept = 0, size = 1, col = "gray70") +
  scale_x_continuous(breaks = seq(40, 105, 5)) +
