...
```{r, eval=FALSE}
library(chillR)
daylength(latitude = 30.39306, JDay = 120)

all_daylengths<-cbind(JDay = 1:365,
                      sapply(daylength(latitude = 30.39306, JDay=1:365),cbind))
#knitr::kable(head(all_daylengths))
head(all_daylengths)
```

Get the data. Tallahassee station id: 72214093805
```{r}
library(dplyr)
library(lubridate)
...
for(i in 2000:2009){
  print(i)
  X <- read.csv(file = paste0("https://www.ncei.noaa.gov/data/local-climatological-data/access/", i, "/72214093805.csv")) %>%
    dplyr::select(DATE, REPORT_TYPE, starts_with("Hourly"))
  df <- rbind(df, X)
}
...
DayHour.df <- df2 %>%
  mutate(Hour = hour(Date),
         ...
         Doy = yday(Date)) %>%
  group_by(Doy, Hour) %>%
  summarize(AvgWindSpeed = median(HourlyWindSpeed, na.rm = TRUE) * .44704)


ggplot(DayHour.df, aes(x = Doy, y = Hour, fill = AvgWindSpeed)) +
  geom_raster() +
  scale_fill_continuous(type = "viridis", alpha = .6, name = "") +
  scale_y_reverse(breaks = seq(1, 23, 3), labels = c("1am", "4am", "7am", "10am", "1pm", "4pm", "7pm", "10pm")) +
  scale_x_continuous(breaks = c(15,	46,	75,	106, 136, 167, 197, 228, 259, 289, 320, 350),
                     labels = month.abb) +
  xlab("") + ylab("") +
  theme_minimal() +
  labs(title = "Median Wind Speed (m/s)", 
       subtitle = "Tallahassee, Florida (2000-2019)", 
       caption = "Data Source: NOAA NCEI")
```
...
To get an indication of how 'consistently' hot Tally is we compute the H-index Treacy, M. M. J., C. N. Ramirez, and M. O’Keeffe (2019), Hotness and coldness indexes based on the Fahrenheit scale, Eos, 100, https://doi.org/10.1029/2019EO127323. Published on 05 July 2019. And find that it is 91.

Tallahassee had an H-index of 91 in 2018: There were 91 days for which the maximum daily temperature was at least 91°F.
```{r}
H2 <- numeric()
for(Y in 1940:2018){
H <- numeric()
...
h <- TLH.df %>% 
  filter(Year == Y) %>%
  summarize(H = sum(TMAX >= Temp)) %>%
#  summarize(H = sum(TMIN >= Temp)) %>%
  pull(H)
H <- c(H, h)
}
H.df <- data.frame(Temp = 80:100, H) 
#H.df <- data.frame(Temp = 57:77, H) 

h2 <- H.df %>%
  mutate(diff = Temp - H < 0) %>%
  group_by(diff) %>%
  mutate(last = last(Temp)) %>%
  mutate(Hindex = nth(last, 1)) %>%
  pull(Hindex) %>%
  first()
H2 <- c(H2, h2)
}

Hindex.df <- data.frame(Year = 1940:2018, H2)

ggplot(Hindex.df, aes(x = Year, y = H2)) +
